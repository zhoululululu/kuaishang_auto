...
class GetRequests:
    ...
    def get_params(api_name, data):
        ...
        # 儿科
        if api_name == "pae_intent":
            params = {
                "utterance": data[0],
                "multi_intent_mod": data[1],
                "enterprise": data[2]
            }

        if api_name == "obstetrics":
            params = {
                "utterance": data
            }
    ...
    def get_request(self, api_url, method, api_category, target_file, test_data_file, testdata_param, assert_param,
        ...
        if api_category == "ner":
            ...
            test_data = ChangeDataType.csv_to_dict(rootPath + "\\testdata\\" + test_data_file)
        ...
        if "similary" in api_category:  # 相似度处理（二分类算法，直接调用输出显示prf值）
            ...
            # file_name = GetRequests.get_target_result(self, test_data_file, exp_list, re_list, target_result_file,
            #                                           total_num, accuracy)
            file_name = GetRequests.get_target_result(self, target_file, exp_list, re_list, target_result_file,
                                                      total_num, accuracy)
            return file_name

    def get_paediatrics_request(self, api_url, method, api_category, test_data_file, testdata_param, assert_param,
                                result_file, target_result_file):
        """
        通过抽取测试集的数据，调用请求api接口，得出的测试结果，在调用函数获取二分类或，每个target的准确率，召回率，F1
        :param target_file: 储存target的文件
        ...
        if "similary" in api_category:  # 相似度处理（二分类算法，直接调用输出显示prf值）
            ...
            file_name = GetRequests.get_paediatrics_target_result(self, test_data_file, exp_list, re_list,
                                                                  target_result_file,
                                                                  total_num, accuracy)
            # file_name = GetRequests.get_target_result(self, target_file, exp_list, re_list, target_result_file,
            #                                           total_num, accuracy)
    ...
    def get_csv_data(self, file_path):
        """
        得到测试数据
        :param file_path: 测试文件（CSV）
        :return:返回测试数据列表
        """
        with open(rootPath + "\\testdata\\apidata\\" + file_path, 'r', encoding='utf-8-sig') as csvfile:
            ...
            # print("从测试数据文件中得到每条测试数据和对应的label**************结束")
        return label_list

    def get_paediatrics_target_result(self, test_data_file, bz_intent_list, re_intent_list, target_result_file,
                                      total_num, accuracy):
        """
        ...
        label_list = self.get_csv_data(test_data_file)
