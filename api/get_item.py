...
class GetItem:
    ...
    def get_item_result(self, target_file, bz_intent_list, re_intent_list):
        """
        ...
        """
        # 获取target列表
        target_list = CommonFunction.get_target(self, "item\\" + target_file)
        ...
        workbook.save(rootPath + '\\testresults\\resultfile\\' + now + "beauty_each_target_result.xls")

    def get_item(self, api_url, target_file, test_data_file, result_file):
        """
        ...
        """
        # 获取测试集的data
        test_data = ChangeDataType.csv_to_dict(rootPath + "\\testdata\\apidata\\item\\" + test_data_file)
        re_item_list = []
        exp_item_list = []
        tf_list = []
        for idx, temp in test_data.iterrows():
            ...
            try:
                ...
                result = r.json()
                re_item = result["data"]["item"]  # 获取返回data的item
                print(re_item)
                tf = CommonFunction.get_tf(item, re_item)
            except Exception as e:
                score = "bad request"
            exp_item_list.append(item)
        ...
        test_data.to_excel(rootPath + '\\testresults\\resultfile\\' + now + result_file, index=False,
                           encoding="utf-8")
        GetItem.get_item_result(self, target_file, exp_item_list, re_item_list)


if __name__ == '__main__':
    GetItem().get_item("http://192.168.1.74:8900/meal?sentence={}", "beauty_item.txt",
                       "beauty_item_testdata.csv",
