...
class CommonFunction:
    ...
    def change_space_to_comma(test_data_file):
        ...
        f1 = open((rootPath + "\\testdata\\apidata\\intent\\" + test_data_file), 'r+', encoding='UTF-8',
                  errors='ignore')
        infos = f1.read()
    ...
    def get_txt_to_csv(test_data_file, result_data_file):
        ...
        # 创建或打开csv文件，以写方式
        with open((rootPath + "\\testdata\\apidata\\ner\\" + result_data_file), 'w+', encoding='UTF-8',
                  newline='') as csvfile:
            spamwriter = csv.writer(csvfile, dialect='excel')
            # 打开txt文件，以读方式
            with open((rootPath + "\\testdata\\apidata\\ner\\" + test_data_file), 'r', encoding='UTF-8',
